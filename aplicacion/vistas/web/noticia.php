...
// 1. Obtener la noticia y su galería usando Eloquent
$noticia = NoticiaModelo::with('imagenesAdjuntas')
                        ->whereIn('estado', [1, 2])
                        ->find($id);
...
// 2. La galería ya viene cargada gracias a la relación en el modelo
$galeria = $noticia->imagenesAdjuntas;

// 3. Obtener 3 noticias relacionadas (estado 1, excluyendo la actual)
$relacionadas = NoticiaModelo::where('estado', 1)
                             ->where('id', '!=', $id)
...
<main class="noticia-main">
    <article class="noticia-articulo">
        ...
        <?php if ($galeria->isNotEmpty()): ?>
        <div class="noticia-galeria">
            <h3><i class="fa-regular fa-images"></i> Galería</h3>
            <div class="galeria-grid">
                <?php foreach($galeria as $img): ?>
                <div class="galeria-item" onclick="abrirLightbox('<?= URL ?><?= htmlspecialchars($img->imagen) ?>')">
                    <img src="<?= URL ?><?= htmlspecialchars($img->imagen) ?>" alt="Imagen de galería">
